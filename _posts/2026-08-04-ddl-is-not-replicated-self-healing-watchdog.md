...
## 곁가지: "레플리카는 read-only인데 복제는 어떻게 쓰지"

작업을 공유하다 팀에서 나온 질문이다. 레플리카는 팀원 계정에 읽기 전용을 걸어뒀는데,
그럼 자동 치유가 실행하는 `ALTER TABLE`은 권한을 풀어야 하는 것 아니냐는.

풀 필요가 없다. 같은 DB 안에서도 SQL을 실행하는 **주체는 여럿**이고 권한은 주체별로 따로 놀기 때문이다.
사람이 접속하는 롤, 애플리케이션 커넥션, 복제 apply worker, pg_cron 잡 — 전부 다른 주체다.
apply worker는 구독 소유자 권한으로 도는 백그라운드 프로세스라 팀원 롤의 설정과 애초에 무관하고,
자동 치유도 pg_cron이 관리자 권한으로 실행한다. "이 DB는 read-only"라는 말은 대개 부정확하고,
정확히는 "그 롤로 접속한 세션이 read-only"다.

한 겹 더 들어가면, 읽기 전용을 만드는 방법도 성격이 둘로 갈린다. `pg_read_all_data`만 주고
쓰기 권한을 아예 안 주는 **GRANT**는 진짜 경계선이라 어떤 방법으로도 못 뚫는다. 반면
`ALTER ROLE ... SET default_transaction_read_only = on`은 세션 기본값이라 세션에서 끌 수 있다 —
보안 경계가 아니라 실수 방지용 안전핀이다. 둘 다 걸어두는 게 정석이고, 이번에 확인해 보니
실제로 그렇게 되어 있었다.

그리고 이 분리는 취향이 아니라 정합성 요구사항이다. 논리복제 대상은 물리 standby와 달리 쓰기가
열려 있는데, 사람이 임의로 넣은 행은 나중에 소스에서 같은 키가 복제될 때 충돌한다. 그 순간
해당 테이블만이 아니라 **구독 전체의 apply가 멈춘다**. 지난 복구 때 겪은 유니크 충돌이 정확히 그 형태였다.
소스 값으로 대상을 맞추는 보정만 예외인데, 그건 충돌을 만드는 게 아니라 없애는 방향이라서다.
...
- "이 DB는 read-only"는 부정확한 문장이다. 권한은 **주체별**(사람 롤 / 앱 / 복제 워커 / 스케줄러)로
  따로 놀고, GRANT는 경계선이지만 `default_transaction_read_only`는 안전핀이다. 둘을 구분하지 않으면
  "막았다고 생각했는데 안 막힌" 구멍이 생긴다.
